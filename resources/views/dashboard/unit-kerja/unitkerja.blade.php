                    ...
                    <div class="box-body">
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th>No.</th>
                                    <th>Nama Unit Kerja Kementerian Kesehatan</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($kerjaList as $data)
                                <tr>
                                  <td>{{$loop->iteration}}</td>
                                  <td>{{$data->kerja}}</td>
                                  <td>
                                    <a href="{{ route('unitkerja.edit', ['id' => $data->id]) }}" class="btn btn-warning">EDIT</a>
                                    <a href="{{ route('unitkerja.delete', ['id' => $data->id]) }}" onclick="return confirm('Ingin menghapus <?= $data->kerja ?>')" class="btn btn-danger">DELETE</a>
                                @endforeach
                            </tbody>
                        </table>
                        {{ $kerjaList->links() }}
